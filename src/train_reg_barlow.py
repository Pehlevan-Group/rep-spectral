...
def train():
    ...
    for i in pbar:
        model.train()
        total_train_loss = 0
        for param_update_count, ((x1, x2), _) in enumerate(train_loader):
            x1, x2 = x1.to(device), x2.to(device)
            opt.zero_grad()

            train_loss = model(x1, x2)
        ...
        if i % args.log_epoch == 0:
            ...
            pbar.set_postfix(
                {'tr_loss': train_loss.item(), "tr_dacc": downstream_train_acc, 'te_dacc': downstream_test_acc})
