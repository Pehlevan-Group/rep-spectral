...
# load files
from .utils import batch_jacobian, derivatives


def top_eig_regularizer_autograd(x: torch.Tensor, feature_map: nn.Module, sample_size: float = None, scanbatchsize: int = 20, max_layer: int=None):
    """
    ...
    for i, layer in enumerate(feature_map):
        if isinstance(layer, nn.Linear): 
            counter += 1
        if max_layer is not None and counter == max_layer: break
...
def top_eig_ub_regularizer_autograd(x: torch.Tensor, feature_map: nn.Module, max_layer: int=None):
    """
