...
# training
parser.add_argument('--batch-size', default=512, type=int, help='the batchsize for training')
parser.add_argument('--lr', default=0.2, type=float, help='learning rate for the last layer')
...
model = ResNet50(num_classes=num_classes, nl=nl).to(device)
...
def get_reg_loss(model, features: torch.Tensor) -> torch.Tensor: 
    """
