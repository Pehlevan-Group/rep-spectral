...
class Attacker:
    ...
    def binary_search(self, x_adv: torch.Tensor, tol: float = 1e-5) -> torch.Tensor:
        ...
        while torch.max((rights - lefts)) > tol:
            mids = (lefts + rights) / 2
            x_mid = (1 - mids) * self.x + mids * x_adv
            success_indicators = self.check_attack_success(
                x_mid, self.y, self.y_targets
            )
        ...
        result = (1 - rights) * self.x + rights * x_adv
        return result
...
class TangentAttack(Attacker):
    ...
    def __init__(
        ...
        self.y = self.get_decision(self.x)  # predicted label

        # adversarial labels
        self.x_targets = x_targets
        self.y_targets = None if x_targets is None else self.get_decision(x_targets)
    ...
    def _initialize(self, max_search_times: int = 5000) -> torch.Tensor:
        """
        ...
        if self.x_targets is None:
            ...
            x_original = self.x
            idx_tensor = torch.arange(self.batch_size).to(self.device)
            x0_inserted, insert_idx = (
                torch.empty(0, *self.shapes).to(self.device),
            ...
            while count < max_search_times:
                ...
                success_indicators = random_noise_pred != self.y[idx_tensor]
            ...
            random_noises = torch.zeros_like(self.x)
            random_noises[insert_idx] = x0_inserted

            # project random noise to decision boundary using binary search
            x0 = self.binary_search(random_noises, self.tol)
        else:
            x0 = self.binary_search(self.x_targets, self.tol)

        return x0

    def _estimate_normal_direction(
        self, x_bounds: torch.Tensor, num_evals: int, delta: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        label_changed = (perturbed_prediction != self.y[:, None, None]).float()
    ...
    def _geometric_progression_for_tangent_point(
        ...
        while True:
            ...
            success_indicators = self.check_attack_success(
                tangent_point,
                self.y[idx_tensor],
                self.y_targets[idx_tensor] if self.y_targets is not None else None,
            )
    ...
    def attack(self) -> torch.Tensor:
        ...
        # initialize
        cur = self._initialize()
        cur_dists = (
            (cur - self.x).view(self.batch_size, -1).norm(p=2, dim=-1, keepdim=True)
        )
        for i in range(1, self.T + 1):
            ...
            normal_vecs = self._estimate_normal_direction(cur, num_evals, deltas)

            # find valid tangent point
            tangent_point = self._geometric_progression_for_tangent_point(
                self.x, cur, normal_vecs, cur_iter=i
            )
            ...
            logging.info(f"Iteration {i}: average distance = {cur_dists.mean():.6f}")
