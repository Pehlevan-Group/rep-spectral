...
def projection(x: torch.Tensor, x_adv: torch.Tensor, norm: Union[str, int], eps: float) -> torch.Tensor:
    ...
    if norm == 'inf':
        ...
        delta *= eps / scaling_factor.view(x.shape[0], *[1 for _ in range(len(x.shape) - 1)])
        x_adv = x + delta
    ...
    for _ in range(k):
        ...
        with torch.no_grad():
            if norm == 'inf':
                ...
                # normalize
                gradients /= gradients.flatten(start_dim=1).norm(norm, dim=-1).view(X.shape[0], *[1 for _ in range(len(X.shape) - 1)])
                gradients *= step_size
